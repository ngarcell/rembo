...
async def register_vehicle(
    ...
    if current_user.role != "manager":
        raise HTTPException(
            status_code=403, detail="Only managers can register vehicles"
        )

    if not current_user.fleet_id:
        raise HTTPException(
            status_code=400, detail="Manager must be assigned to a fleet"
        )
...
async def list_vehicles(
    ...
    vehicle_summaries = [
        VehicleSummaryResponse(**vehicle.to_summary_dict()) for vehicle in vehicles
    ]
...
async def get_vehicle_details(
    ...
    if current_user.role != "manager":
        raise HTTPException(
            status_code=403, detail="Only managers can view vehicle details"
        )

    # Get vehicle
    vehicle = VehicleService.get_vehicle_by_id(
        vehicle_id=vehicle_id, manager_id=str(current_user.id), db=db
    )
