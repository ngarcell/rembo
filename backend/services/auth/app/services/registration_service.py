...
class RegistrationService:
    ...
    def initiate_registration(self, phone: str, db: Session) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        try:
            ...
            resend_available_at = datetime.utcnow() + timedelta(minutes=1)  # 1 minute cooldown
            ...
            if self.supabase:
                try:
                    auth_response = self.supabase.auth.admin.create_user({
                        "phone": phone,
                        "email": email if email else f"{phone.replace('+', '')}@temp.matatu.app",
