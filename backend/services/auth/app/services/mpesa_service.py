...
class MpesaService:
    ...
    def generate_password(self) -> str:
        ...
        password_string = (
            f"{self.business_short_code}{self.lipa_na_mpesa_passkey}{timestamp}"
        )
    ...
    async def initiate_stk_push(
        self, phone_number: str, amount: Decimal, booking_id: str, db: Session
    ) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    self.stk_push_url, json=stk_request, headers=headers
                )
                ...
                if (
                    ...
                    and response_data.get("ResponseCode") == "0"
                ):
                    # Success - update payment transaction
                    payment_transaction.checkout_request_id = response_data.get(
                        "CheckoutRequestID"
                    )
                    payment_transaction.merchant_request_id = response_data.get(
                        "MerchantRequestID"
                    )
                    ...
                    logger.info(
                        f"STK Push initiated successfully for payment {payment_reference}"
                    )
                    ...
                    logger.error(
                        f"STK Push failed for payment {payment_reference}: {error_message}"
                    )
    ...
    async def query_stk_push_status(
        self, checkout_request_id: str, db: Session
    ) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    self.stk_query_url, json=query_request, headers=headers
                )
    ...
    async def handle_stk_callback(
        self, callback_data: Dict, db: Session
    ) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if result_code == 0:  # Success
                ...
                logger.info(
                    f"Payment completed successfully: {payment.payment_reference}"
                )
                ...
                logger.info(
                    f"Payment failed: {payment.payment_reference} - {result_desc}"
                )
