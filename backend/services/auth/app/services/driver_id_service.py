...
class DriverIDService:
    ...
    @staticmethod
    def generate_driver_id(fleet_id: str, db: Session) -> Tuple[bool, str, Optional[str]]:
        """
        ...
        try:
            ...
            fleet_code = DriverIDService._generate_fleet_code(fleet.fleet_code or fleet.name)
            ...
            if next_number > 999:
                return False, "", "Fleet has reached maximum driver capacity (999 drivers)"
            
            # Format driver ID
            driver_id = f"DRV-{next_number:03d}{fleet_code}"
            
            # Verify uniqueness (should be unique by design, but double-check)
            existing = db.query(DriverProfile).filter(DriverProfile.driver_id == driver_id).first()
            if existing:
    ...
    def _generate_fleet_code(fleet_name: str) -> str:
        ...
        words = fleet_name.upper().replace("FLEET", "").replace("SACCO", "").replace("MATATU", "").split()
    ...
    def _get_next_driver_number(fleet_id: str, db: Session) -> int:
        ...
        try:
            ...
            count = db.query(DriverProfile).filter(DriverProfile.fleet_id == fleet_id).count()
