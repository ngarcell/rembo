...
class VehicleRegistrationRequest(BaseModel):
    ...
    fleet_number: str = Field(
        ...
        min_length=1,
        max_length=20,
        description="Fleet number (unique within fleet)",
    )
    license_plate: str = Field(
        ...,
        min_length=1,
        max_length=20,
        description="License plate (unique across system)",
    )
    ...
    gps_device_id: Optional[str] = Field(
        None, max_length=100, description="GPS device identifier"
    )
    sim_number: Optional[str] = Field(
        None, max_length=20, description="SIM card number"
    )
    gps_api_key: Optional[str] = Field(
        None, description="GPS API key (will be encrypted)"
    )
    ...
    def validate_license_plate(cls, v: str) -> str:
        ...
        if not any(re.match(pattern, v) for pattern in patterns):
            raise ValueError(
                "Invalid license plate format. Expected Kenyan format (e.g., KCA123A)"
            )
    ...
    def validate_fleet_number(cls, v: str) -> str:
        ...
        if not re.match(r"^[A-Za-z0-9\-_]+$", v):
            raise ValueError(
                "Fleet number can only contain letters, numbers, hyphens, and underscores"
            )
...
class VehicleUpdateRequest(BaseModel):
    ...
    validate_license_plate = field_validator("license_plate")(
        VehicleRegistrationRequest.validate_license_plate
    )
    validate_fleet_number = field_validator("fleet_number")(
        VehicleRegistrationRequest.validate_fleet_number
    )
    validate_sim_number = field_validator("sim_number")(
        VehicleRegistrationRequest.validate_sim_number
    )
