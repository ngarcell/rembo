...
async def test_stk_push():
    ...
    try:
        ...
        success, result = await mpesa_service.initiate_stk_push(
            phone_number=test_phone, amount=amount, booking_id=booking_id, db=db
        )
        ...
        if success:
            ...
            if result.get("checkout_request_id"):
                status_success, status_result = (
                    await mpesa_service.query_stk_push_status(
                        checkout_request_id=result["checkout_request_id"], db=db
                    )
                )
            ...
            payment = (
                ...
                .filter(PaymentTransaction.id == result["payment_id"])
                .first()
            )
